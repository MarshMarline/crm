...
<script type="text/javascript">
	...
	$(function(){
		...
		$.ajax({
			...
			success:function (res) {
				...
				//从模态窗口中得到所有者姓名显示在详细信息页面
				$("#clueowner").text($("option[value='${clue.owner}']").text());
			},
			error:errorfun
		});
		...
		$("#qx").change(function () {
			...
			$("#onbundbtn").prop("disabled",!$(":checkbox[name=xz]:checked").size()==1);
		});
	...
	function getRelativeActivity() {
		$.ajax({
			type:'get',
			url:'workbench/clue/getRelativeActivity.do',
			data:{"id":"${clue.id}"},
			success:function (res) {
				if(res!=null){
					var html = "";
	...
	function removeRelation(id) {
		...
		$.ajax({
			type:'post',
			url:"workbench/clue/removeRelationByReId.do",
			data:{"id" : id},
			success:function (res) {
				if(res.flag){
					$("#tr"+id).remove();
	...
	function btnrefresh() {
		$("#onbundbtn").prop("disabled",!$(":checkbox[name=xz]:checked").size()==1);
		$("#qx").prop("checked",$(":checkbox[name=xz]").size() == $(":checkbox[name=xz]:checked").size());
	}
...
<body>
		...
		<div class="modal-dialog" role="document" style="width: 80%;">
			<div class="modal-content">
					...
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<form class="form-inline" role="form">
						  <div class="form-group has-feedback">
						    <input type="text" class="form-control" id="serach-name" style="width: 300px;" placeholder="请输入市场活动名称，支持模糊查询" >
						    <span class="glyphicon glyphicon-search form-control-feedback" id="show-res"></span>
						  </div>
						</form>
					</div>
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" data-dismiss="modal" id="onbundbtn" disabled>关联</button>
				</div>
			</div>
		</div>
	</div>
	...
	<div style="position: relative; top: -70px;">
		<div style="position: relative; left: 40px; height: 30px;">
			...
			<div style="width: 300px;position: relative; left: 650px; top: -60px;"><b id="clueowner">zhangsan</b><b>&nbsp;</b></div>
